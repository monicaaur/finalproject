  ...
  const getPostData = async () => {
    ...
    .then((explore) => {
      const requests = explore.data.data.posts.map(async (post) => {
        ...
        return Promise.all([postDetails, postLike]);
      })

      Promise.all(requests)
      .then((responses) => {
      setPostData(responses.map(([postDetails, postLike]) => {
          ...
          <>
            <div className="post_wrapper">
              <div className="user_wrapper">
                ...
                <p className="username_text">{posts.user.username}</p>
              </div>
              ...
              <div className="post_caption_wrap">
                <p className="post_username">{posts.user.username}  <span className='caption'>{posts.caption}</span></p>
                <p className="create_date">Created at: {posts.createdAt}</p>
              </div>
            </div>

            <Modal show={show === posts.id} onHide={handleClose} size='lg' aria-labelledby="contained-modal-title-vcenter" centered>
              ...
              <Modal.Body className='comment_modal_body'>
                      ...
                      <>
                        <div className="comment_wrap">
                          ...
                          <div className="comment_section">
                            <a href='' className="comment_username">{comments.user.username}</a>
                ...
                <Comment postId={posts.id} setPostData={setPostData}/>
